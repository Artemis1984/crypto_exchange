...
links = [
         'https://api.binance.com/api/v3/depth?symbol=LTCUSDT&limit=10',
         'https://api.huobi.pro/market/depth?symbol=ltcusdt&type=step0',
         'https://api-pub.bitfinex.com/v2/book/tLTCUST/P0',
         'https://api.kraken.com/0/public/Depth?pair=LTCUSDT',
         'https://global-openapi.bithumb.pro/market/data/orderBook?symbol=LTC-USDT',
         'https://api.kucoin.com/api/v1/market/orderbook/level2_20?symbol=LTC-USDT&limit=10'
...
if __name__ == '__main__':
    while True:
            ...
            if 'binance' in i:
                ...
                for k in data_list[i]:
                    if k[2] < 0:
                        ...
                        ask_list['bitfinex'].append(abs(float(data_list[i][data_list[i].index(k)][2])))
                        bid_list['bitfinex'] = []
                ...
                try:
                    ask_list['kraken'] = []
                    ask_list['kraken'].append(float(data_list[i]['result']['LTCUSDT']['asks'][0][0]))
                    ask_list['kraken'].append(float(data_list[i]['result']['LTCUSDT']['asks'][0][1]))
                except:
                    ask_list['kraken'] = []
                    ask_list['kraken'].append(float(data_list[i]['result']['LTC/USDT']['asks'][0][0]))
                    ask_list['kraken'].append(float(data_list[i]['result']['LTC/USDT']['asks'][0][1]))
                try:
                    bid_list['kraken'] = []
                    bid_list['kraken'].append(float(data_list[i]['result']['LTCUSDT']['bids'][0][0]))
                    bid_list['kraken'].append(float(data_list[i]['result']['LTCUSDT']['bids'][0][1]))
                except:
                    bid_list['kraken'] = []
                    bid_list['kraken'].append(float(data_list[i]['result']['LTC/USDT']['bids'][0][0]))
                    bid_list['kraken'].append(float(data_list[i]['result']['LTC/USDT']['bids'][0][1]))
            elif 'huobi' in i:
                try:
                    ask_list['huobi'] = []
                ...
                data_list[i]['info']['s'] = [k for k in data_list[i]['info']['s'] if float(k[1])]
                data_list[i]['info']['b'] = [k for k in data_list[i]['info']['b'] if float(k[1])]
                try:
                    ask_list['bithumb'] = []
        ...
        if max_profit < clear_profit:
            with open('max_price_LTC.txt') as f:
            ...
            with open('max_price_LTC.txt', 'w') as f:
